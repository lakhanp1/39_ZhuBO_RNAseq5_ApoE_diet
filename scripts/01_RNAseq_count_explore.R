...
outDir <- here("analysis", "01_count_data", analysisName)
...
plotPCA(rld, intgroup=c("condition"), ntop = 4000)
...
# 
# rawDat <- counts(dds)
# keep <- rowSums(rawDat) > 1
# rawDat <- rawDat[keep, ]
# 
# dds$batch[which(dds$batch == "b3")] <- "b2"
# adjusted <- ComBat_seq(counts = rawDat, batch = as.character(dds$batch), group = dds$condition)
# 
# dds2 <- DESeqDataSetFromMatrix(countData = adjusted, colData = colData(dds), design = design)
# dds2 <- DESeq(dds2)
...
# plotPCA(rld2)
# 
# estimateSizeFactorsForMatrix(adjusted)
# 
# ###################
# vsd <- vst(dds)
# vsd$batch[which(vsd$batch == "b3")] = "b2"
# plotPCA(vsd, intgroup=c("treatment", "time"), ntop = 4000)
# 
# assay(vsd) <- limma::removeBatchEffect(
#   x = assay(vsd), batch = vsd$batch,
#   design = matrix(as.numeric(vsd$condition))
# )
# 
# plotPCA(vsd, intgroup=c("treatment", "time"), ntop = 4000)
# 
# keep <- rowSums(counts(dds)) > 1
# gpca <- glmpca(Y = counts(dds)[keep, ], L = 5)
...
# shapeColumn <- "time"
# 
# ggplot(data = gpca.dat, mapping = aes(x = dim1, y = dim2)) +
...
#   geom_text_repel(mapping = aes(label = sampleId), size = 4,
...
pcaData <- plotPCA(rld, intgroup = c("condition", "genotype", "diet"),
...
pcaData$diet <- forcats::as_factor(pcaData$diet)
...
pointCol <- c("ND" = "black", "WD" = "red")
shapeCode <- c("ApoE_del" = 1, "ApoE_del_liver_DKO" = 17)
...
plotData$diet <- forcats::as_factor(plotData$diet)
...
pt_pcScatter <- GGally::ggpairs(
  ...
  lower = list(continuous = wrap("points", size = 3, stroke = 1)),
  upper = "blank",
...
pt_dist <- ComplexHeatmap::Heatmap(
  ...
  row_labels = paste(exptInfo$condition, "_", exptInfo$replicate, ".", exptInfo$sampleId, sep = ""),
...
rownames(samplePoisDistMatrix) <- paste(dds$condition, "_", dds$replicate, ".", dds$sampleId, sep = "")
colnames(samplePoisDistMatrix) <- paste(dds$condition, "_", dds$replicate, ".", dds$sampleId, sep = "")
